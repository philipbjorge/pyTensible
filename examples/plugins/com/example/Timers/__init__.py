    ...
    def __init__(self, how_long, *events):
        ...
        for event in events:
            if not isinstance(event, Events.IEvent):
                raise TypeError(
                    "Expected an event object implementing the IEvent interface. Got an object of %s class." % event.__class__.__name__)
        self._events = events[:]
...
class RTimerManager(ITimerManager, threading.Thread):
    ...
    def add_timer(self, timer):
        if not isinstance(timer, ITimer):
            raise TypeError("Expected a timer object implementing the ITimer interface. Got an object of %s class." %
