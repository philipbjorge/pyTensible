...
from Errors import UnavailableResource
import Logging
...
class Accessor:
    ...
    def __getattr__(self, name):
        try:
            target_self = get_nested_dictionary(
                self.__namespace, self.__namespace_hierarchy)
            if isinstance(target_self, Namespace):
                return target_self.__getattr__(name)
        except KeyError:
            raise UnavailableResource('.'.join(self.__namespace))

        try:
            target = get_nested_dictionary(self.__namespace + [
                                           name], self.__namespace_hierarchy)
        except KeyError:
            raise UnavailableResource('.'.join(self.__namespace + [name]))

        if isinstance(target, Namespace):
            return target
        else:
            sub_accessor = Accessor(self.__namespace + [
                                    name], self.__namespace_hierarchy)
            return sub_accessor
...
class TestAccessors(unittest.TestCase):
    ...
    def setUp(self):
        self.broken_namespace = Namespace({
                                          'IName': "IName Interface"}, {'Name': "Name class"})
        self.alphabet_namespace = Namespace({
                                            'ILetter': "ILetter Interface"}, {'Letter': "Letter class"})

        self.test_accessor = Accessor([], {'foo': {
                                      'bar': {'baz': self.broken_namespace}}})
    ...
    def test_third(self):
        self.assertEqual(
            self.test_accessor.foo.bar.baz.IName, "IName Interface")
