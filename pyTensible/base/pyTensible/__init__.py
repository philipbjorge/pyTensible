...
class PluginLoader(IPluginLoader):
    ...
    # each is keyed by fully qualified symbolic_name eg
    # self._manifests["com.example.Events"] = manifest
    _manifests = {}

    # These are the plug-in modules
    # each is keyed by a fully qualified symbolic_name eg
    # self._plugin_modules["com.example.Events"] = plugin_module
    _plugin_modules = {}

    # These are the plug-in classes.
    # each is keyed by a fully qualified symbolic_name eg
    # self._plugin_classes["com.example.Events"] = plugin_class
    _plugin_classes = {}

    # These are the plug-in objects
    # each is keyed by a fully qualified symbolic_name eg
    # self._plugin_objects["com.example.Events"] = plugin_object
    _plugin_objects = {}
    ...
    # keyed as follows:
    # self._provider_hierarchy['com.example.Events.IEventManager']['com.example.Events.EventManager']
    # = EventManager
    _provider_hierarchy = {}

    # This holds dictionaries of resources which implement specific interfaces,
    # keyed as follows:
    # self._instance_hierarchy['com.example.Events.IEventManager']['com.example.Events.event_manager']
    # = event_manager
    _instance_hierarchy = {}

    # this holds the manifests of each provider keyed by the fully qualified symbolic name
    # each is keyed by a fully qualified symbolic_name eg
    # self._provider_manifests["com.example.IEvents"] = manifest
    _provider_manifests = {}
    ...
    def load_plugins(self, plugins_path, local_suppress_list=[], ):
        ...
        if not os.path.isdir(plugins_path):
            raise InvalidPluginDirectory(
                "Given path is not a directory: " + plugins_path)
    ...
    def _log_exc(self, message, exc_info):
        ...
        message = "Traceback (most recent call last):\n" + ''.join(traceback.format_tb(tb)) + '\n'.join(
            traceback.format_exception_only(sys.exc_type, sys.exc_value))  # @UndefinedVariable
        self.logger.error(message)
        ...
        for directory in os.listdir(plugins_path):
            ...
            if os.path.isdir(sub_directory):
                ...
                if os.path.exists(manifest_path):
                    # it's a plug-in
                    # initializes the manifest, checks that the namespaces
                    # match, then adds the manifest to the manifests dictionary
                    if self._preprocess_plugin(manifest_path, new_namespace):
                        new_plugin_list.append('.'.join(new_namespace))
                else:
                    # it's a sub-directory
                    new_plugin_list += self._preprocess_plugins(
                        sub_directory, new_namespace)
            else:
                if directory[:len("__init__.py")] != "__init__.py":
                    self.logger.debug("Unknown file in plug-in directory: " + str(
                        os.path.join(plugins_path, directory)))
    ...
    def _preprocess_plugin(self, manifest_path, new_namespace):
        try:
                ...
                for resource in manifest.resources_provided:
                    ...
                    if resource_interface is not None:
                        ...
                        if len(resource_interface_namespace) < 2:
                            fully_qualified_interface = manifest.symbolic_name + \
                                '.' + resource_interface
                        else:
                        ...
                        if fully_qualified_interface not in self._provider_manifests.keys():
                            self._provider_manifests[
                                fully_qualified_interface] = []

                        self._provider_manifests[
                            fully_qualified_interface].append(manifest)
                return True
            else:
            ...
            self.logger.error(
                'Uncaught exception occurred reading plug-in manifest.mf file.')
    ...
    def _load_plugins(self, plugins_path, plugin_list, local_suppress_list):
        ...
        for symbolic_name in plugin_list:
            ...
            try:
                self._load_plugin(plugins_path, symbolic_name, None,
                                  depend_list, self._suppress_list + local_suppress_list)
            except UnsatisfiedDependency as e:
                self.logger.error(
                    "Ignore loading: " + symbolic_name + " -- unsatisfied dependency: " + str(e))
            except UnavailableResource as e:
                self.logger.error(
                    "Failed loading: " + symbolic_name + " -- unsatisfied dependency: " + str(e))
            except MalformedPlugin as e:
                self.logger.error(
                    "Failed loading: " + symbolic_name + " -- failed loading")  # dependency: " + str(e))
            except FailedDependency as e:
                self.logger.error(
                    "Failed loading: " + symbolic_name + " -- dependency failed loading: " + str(e))
            except InvalidResourceComponent as e:
                self.logger.error("Failed loading: " + symbolic_name + " required resource: " +
                                  e.resource + " did not provide a valid " + e.componentType + e.component)
    ...
    def _load_plugin(self, plugins_path, symbolic_name, dependency, depend_list, suppress_list):
        ...
        if not symbolic_name in suppress_list:
            self._process_plugin(
                plugins_path, symbolic_name, dependency, depend_list, suppress_list)
        else:
            self.logger.info(
                "Ignore loading: " + symbolic_name + " -- present in suppress list.")

    def _process_plugin(self, plugins_path, symbolic_name, dependency, depend_list, suppress_list):
        # depend_list holds the list of dependencies along the depth-first
        # cross section of the tree. Used to find cycles.
        ...
        # to check whether the dependency can actually be satisfied by loading
        # this plug-in
        if dependency != None:
            if dependency.satisfied(manifest.symbolic_name, manifest.version):
                ...
                raise UnsatisfiedDependency(
                    symbolic_name + ":" + dependency.dependency_range + ". version present is: " + manifest.version)

        if not manifest.symbolic_name in self._plugin_modules.keys():
            # load the dependencies
            self._load_dependencies(
                plugins_path, manifest, depend_list, suppress_list)
            ...
            try:
                ...
                plugin_module = self._load_plugin_module(
                    plugins_path, manifest.symbolic_name)
                ...
                self.logger.error(traceback.format_exc())
                return
                # raise MalformedPlugin(manifest.symbolic_name + ": failed to
                # import.")
            ...
            try:
                ...
                self._failed_list.append(manifest.symbolic_name)
                self.logger.error(
                    manifest.symbolic_name + ": plug-in class is not present (%s)." % class_name)
            ...
            if not issubclass(plugin_class, self._namespace_hierarchy['pyTensible'].Plugin):
                self._failed_list.append(manifest.symbolic_name)
                self.logger.error(
                    manifest.symbolic_name + ": plug-in class is not derived from the pyTensible.Plugin base class.")
                raise MalformedPlugin()
            ...
            try:
                ...
                self.logger.error(
                    'Uncaught exception occurred in plug-in object instantiation.')
                self.logger.error(traceback.format_exc())
                raise MalformedPlugin()
            ...
            try:
                ...
                tb = exceptionTraceback.tb_next
                self.logger.error(
                    manifest.symbolic_name + ": Uncaught exception occurred in plug-in class load method.")
            ...
            try:
                ...
                self.logger.error(
                    'Uncaught exception occurred processing plug-in exported resources.')
                self.logger.error(traceback.format_exc())
                raise MalformedPlugin()
    ...
    def _load_plugin_module(self, plugins_path, symbolic_name):
        ...
        if not os.path.isfile(plugin_file):
            self.logger.error(
                "The __init__.py file for the following plug-in is missing: %s." % symbolic_name)
            raise MalformedPlugin(symbolic_name + ": failed to import.")

        try:
            add_to_hierarchical_dictionary(
                module_namespace, {}, self._namespace_hierarchy)
        except KeyError:
            pass
        ...
        try:
            ...
            try:
                plugin_module = imp.load_module(
                    symbolic_name, fp, plugin_file, description)
            except:
                self._log_exc("An error occurred on import of {}.".format(
                    symbolic_name), sys.exc_info())
                raise MalformedPlugin(symbolic_name + ": failed to import.")
    ...
    def _update_modules(self, symbolic_name):
        ...
        for i in range(1, len(namespace_hierarchy)):
            ...
            if not symbolic_prefix in sys.modules.keys():
                sys.modules[symbolic_prefix] = Accessor(
                    namespace_hierarchy[:i], self._namespace_hierarchy)
    ...
    def _process_exported_resources(self, manifest, exported_resources):
        ...
        for interface in manifest.interfaces_provided:
            if not interface in interfaces_exported.keys():
                raise MalformedPlugin("The manifest(%s) states that the interface '%s' will be exported but it was not." % (
                    manifest.symbolic_name, interface))

            # add_to_hierarchical_dictionary(namespace_hierarchy,
            # interfaces_exported[interface], self._interface_hierarchy)

        for resource in manifest.resources_provided:
            if not resource['resource_symbolic_name'] in resources_exported.keys():
                raise MalformedPlugin("The manifest states that the resource '%s' will be exported but it was not." % resource[
            ...
            if resource_interface != None:
                ...
                if resource_interface in interfaces_exported.keys():
                    ...
                    interface_namespace = '.'.join(
                        resource_interface.split('.')[:-1])
                    interface_name = resource_interface.split('.')[-1]

                    try:
                        interface = sys.modules[
                            interface_namespace].__getattr__(interface_name)
                    except (IndexError, AttributeError):
                        raise MalformedPlugin("The manifest states that the resource '%s' will provide or implement the interface '%s' but the specified interface could not be found." % (
                            resource['resource_symbolic_name'], resource_interface))

                # add_to_hierarchical_dictionary(namespace_hierarchy,
                # resources_exported[resource['resource_symbolic_name']],
                # self._resource_hierarchy)

                if not local:
                    fully_qualified_interface_name = resource[
                        'resource_interface']
                else:
                    fully_qualified_interface_name = manifest.symbolic_name + \
                        '.' + resource['resource_interface']

                fully_qualified_resource_name = manifest.symbolic_name + \
                    '.' + resource['resource_symbolic_name']

                if resource['resource_type'] == type:
                    if not issubclass(resources_exported[resource['resource_symbolic_name']], interface):
                        raise MalformedPlugin("The manifest states that the resource class '%s' will implement the interface '%s' but it does not." % (
                            resource['resource_symbolic_name'], interface))

                    if not fully_qualified_interface_name in self._provider_hierarchy.keys():
                        self._provider_hierarchy[
                            fully_qualified_interface_name] = {}

                    self._provider_hierarchy[fully_qualified_interface_name][
                        fully_qualified_resource_name] = resources_exported[resource['resource_symbolic_name']]

                elif resource['resource_type'] == object:
                    if not isinstance(resources_exported[resource['resource_symbolic_name']], interface):
                        raise MalformedPlugin("The manifest states that the resource object '%s' will implement the interface '%s' but it does not." % (
                            resource['resource_symbolic_name'], interface))
        ...
        add_to_hierarchical_dictionary(
            namespace_hierarchy, namespace, self._namespace_hierarchy)
    ...
    def _load_interfaces(self, plugins_path, manifest, depend_list, suppress_list):
        ...
        for interface_dependency in manifest.interfaces_implemented:
            ...
            if interface_dependency.dependency_name in self._failed_list:
                self.logger.error("The plug-in providing the interface, " +
                                  interface_dependency.dependency_name + ", previously failed to load.")
            # else load the provider
            else:
                try:
                    self._load_plugin(
                        plugins_path, interface_dependency.dependency_name, interface_dependency, depend_list, suppress_list)

                except KeyError:
                    self.logger.error("Loading the interface, " + interface_dependency.dependency_name +
                                      " failed: unknown (interface missing from interfaces dictionary).")
                    raise UnsatisfiedInterface(
                        interface_dependency.dependency_name + " was not loaded correctly.")
                except UnsatisfiedDependency:
                    self.logger.error(
                        "Loading the interface, " + interface_dependency.dependency_name + " failed: missing dependencies.")
                except MalformedPlugin:
                    self.logger.error(
                        "Loading the interface, " + interface_dependency.dependency_name + " failed: malformed plug-in.")
    ...
    def _load_dependencies(self, plugins_path, manifest, depend_list, suppress_list):
        for dependency in manifest.dependencies:
            ...
            # else load the plug-in
                ...
                self._load_plugin(
                    plugins_path, dependency.dependency_name, dependency, depend_list, suppress_list)
        ...
        for request in manifest.requests:
            for provider in self._get_provider_manifests(request.dependency_name):
                ...
                if provider.symbolic_name in depend_list:
                    # append the name here for showing the cycle in the
                    # exception
                ...
                if provider.symbolic_name in self._failed_list:
                    self.logger.error(
                        provider.name + " a " + provider.Provides + " provider previously _failed to load.")
                # else load the provider
                else:
                    try:
                        self._load_plugin(
                            plugins_path, provider.symbolic_name, None, depend_list, suppress_list)
                    except UnsatisfiedDependency:
                        self.logger.error(
                            provider.name + " a " + provider.Provides + " provider failed: missing dependencies.")
                    except MalformedPlugin:
                        self.logger.error(
                            provider.name + " a " + provider.Provides + " provider failed: malformed plug-in.")
...
def add_to_hierarchical_dictionary(hierarchy, item, dictionary):
    ...
    if len(hierarchy) > 0:
        ...
        if not key in dictionary.keys():
            ...
            raise KeyError(
                "That target key is already in the hierarchical dictionary.")
