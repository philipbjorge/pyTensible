...
class Manifest(object):
    ...
    def __init__(self, manifest_path, expected_namespace):
        ...
        if self.symbolic_name != '.'.join(expected_namespace):
            raise MalformedManifest("Plug-in stated SymbolicName, '%s', does match expected namespace of '%s'." % (
                self.symbolic_name, '.'.join(expected_namespace)))
        ...
        for interface_name, temp in interfaces_provided:  # @UnusedVariable
            if '.' in interface_name:
                raise MalformedManifest(
                    "Names of interfaces provided may not have '.' in them.")
        ...
        for resource_description, temp in resources_provided:  # @UnusedVariable
            ...
            # returns a dictionary {'resource_symbolic_name':
            # resource_symbolic_name, 'resource_interface': resource_interface,
            # 'resource_type': resource_type}
            resource_provided = self._process_resource_provided(
                resource_description)

            if '.' in resource_provided['resource_symbolic_name']:
                raise MalformedManifest(
                    "Names of resources provided may not have '.' in them.")
        ...
        for dependency_name, dependency_string in interfaces_implemented:
            ...
            if len(dependency_namespace) < 2:
                raise MalformedManifest(
                    "The interface dependency specified '%s' is to short to have an enclosing namespace and an interface name." % dependency_name)
            ...
            dependency = Dependency.Dependency(
                dependency_name, dependency_string)
    ...
    def _process_resource_provided(self, description):
        ...
        if description.find('(') != -1:
            ...
            if interface_start == 0:
                raise MalformedManifest(
                    "Found zero length resource name in resource description '%s'" % description)

            resource_symbolic_name = description[:interface_start]

            if interface_end == -1:
                raise MalformedManifest(
                    "Found no closing parenthesis in resource description '%s'" % description)
